...
import argparse
import numpy as np
from utils.data_loader_lilim import DataLoader
import glob
...
parser.add_argument('--file_path', default='lilim_PDPTW_train', help='')
parser.add_argument('--eval_file_path', default='lilim_PDPTW_val', help='')
...
while epoch < args.n_epoch:
    ...
    rank_train1 = [[] for _ in range(6)]
    rank_train2 = [[] for _ in range(6)]

    net.train()
    dataset_index = epoch % 6
    dataLoader.load_data(dataset_index)
    for batch in trange(160 * 6):
        ...
        rank_train1[batch % 6].append((rank_batch1.reshape(-1) * label1.reshape(-1)).sum() / label1.sum())
        ...
        rank_train2[batch % 6].append((rank_batch2.reshape(-1) * label2.reshape(-1)).sum() / label2.sum())
    print ("Epoch {} loss {:.7f} rank:".format(epoch, np.mean(statistics["loss_train"])), ",".join([str(np.mean(rank_train1[_]) + 1)[:5] + " " + str(np.mean(rank_train2[_]) + 1)[:5]  for _ in range(6)]))

    if epoch % args.eval_interval == 0:
        eval_results = []
        for n_nodes in [100]:
            dataset = pickle.load(open(args.eval_file_path + "/" + str(n_nodes) + "_68_101.pkl", "rb"))
            rank_test1 = []
            rank_test2 = []
            for eval_batch in trange(100 // args.eval_batch_size):
        ...
        print ("n=100 %.3f %.3f" % (tuple(eval_results)))
