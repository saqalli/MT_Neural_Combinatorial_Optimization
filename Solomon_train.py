...
import argparse
import numpy as np
from utils.data_loader_sol import DataLoader
import glob
...
parser.add_argument('--file_path', default='sol_CVRPTW_train', help='')
parser.add_argument('--eval_file_path', default='sol_CVRPTW_val', help='')
...
while epoch < args.n_epoch:
    ...
    rank_train1 = [[] for _ in range(7)]
    rank_train2 = [[] for _ in range(7)]

    net.train()
    dataset_index = epoch % 7
    dataLoader.load_data(dataset_index)
    for batch in trange(140 * 7):
        ...
        rank_train1[batch % 7].append((rank_batch1.reshape(-1) * label1.reshape(-1)).sum() / label1.sum())
        ...
        rank_train2[batch % 7].append((rank_batch2.reshape(-1) * label2.reshape(-1)).sum() / label2.sum())
    print ("Epoch {} loss {:.7f} rank:".format(epoch, np.mean(statistics["loss_train"])), ",".join([str(np.mean(rank_train1[_]) + 1)[:5] + " " + str(np.mean(rank_train2[_]) + 1)[:5]  for _ in range(7)]))

    if epoch % args.eval_interval == 0:
        eval_results = []
        for n_nodes in [100]:
            dataset = pickle.load(open(args.eval_file_path + "/" + str(n_nodes) + "_240_70.pkl", "rb"))
            rank_test1 = []
            rank_test2 = []
