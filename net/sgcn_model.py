...
class SparseGCNModel(nn.Module):
    def __init__(self, hidden_dim=128, n_gcn_layers=30, n_mlp_layers=3, problem="tsp"):
        ...
        if problem == "tsp":
            self.node_dim = 2 # x, y
        elif problem == "cvrp" or problem=="vrp":
            ...
            self.node_dim = 9 # x, y, demand, start_time, end_time, capacity, depot, pickup, delivery
        else:
            assert False
        self.edge_dim = 1
        ...
        for layer in range(self.n_gcn_layers):
            gcn_layers.append(SparseGCNLayer(self.hidden_dim, self.aggregation, is_pdp=(problem=="pdp" or problem=="pdptw")))
        self.gcn_layers = nn.ModuleList(gcn_layers)
        if problem == "pdp" or problem == "cvrptw" or problem == "pdptw":
