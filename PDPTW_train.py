...
parser.add_argument('--file_path', default='PDPTW_train', help='')
parser.add_argument('--eval_file_path', default='PDPTW_val', help='')
...
while epoch < args.n_epoch:
    ...
    rank_train1 = [[] for _ in range(26)]
    rank_train2 = [[] for _ in range(26)]

    net.train()
    dataset_index = epoch % 6
    dataLoader.load_data(dataset_index)
    for batch in trange(40 * 26):
        ...
        rank_train1[batch % 26].append((rank_batch1.reshape(-1) * label1.reshape(-1)).sum() / label1.sum())
        ...
        rank_train2[batch % 26].append((rank_batch2.reshape(-1) * label2.reshape(-1)).sum() / label2.sum())
    print ("Epoch {} loss {:.7f} rank:".format(epoch, np.mean(statistics["loss_train"])), ",".join([str(np.mean(rank_train1[_]) + 1)[:5] + " " + str(np.mean(rank_train2[_]) + 1)[:5]  for _ in range(26)]))
