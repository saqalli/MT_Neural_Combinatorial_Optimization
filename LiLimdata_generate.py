...
def write_instance(instance, instance_name, instance_filename):
    with open(instance_filename, "w") as f:
        ...
        for l in range(n_nodes):
            f.write(" "+str(l+1)+" "+str(x[l][0]*140)[:15]+" "+str(x[l][1]*140)[:15]+"\n")
        f.write("PICKUP_AND_DELIVERY_SECTION\n")
        f.write("1 0 0 10000000 0 0 0\n")
        for i in range(n_nodes - 1):
            if i < n_nodes// 2:
                f.write(str(i+2)+ " " + str(int(demand[i]))+ " " + str(float(a[i])) + " " + str(float(b[i])) +
                 " " + str(service_time) + " 0 "+str(i+2+(n_nodes )//2) + "\n")
            else:
                f.write(str(i+2)+ " " + str(int(-demand[i-n_nodes//2]))+ " " + str(float(a[i])) + " " + str(float(b[i])) +
                 " " + str(service_time) + " " + str(i+2-n_nodes//2) + " 0\n")
...
def solve_LKH(dataset_name, instance, instance_name, rerun=False, max_trials=1000):
    ...
    instance_filename = "tmp/" + dataset_name + "/lilim_pdptw/" + instance_name + ".pdptw"
...
def generate_dataset(n_samples, n_nodes, save_dir,mu,sig):
    ...
    n_neighbours = 20
    os.makedirs("tmp/" + str(n_nodes) + "_" + str(mu)+ "_" + str(sig) + "/lilim_pdptw", exist_ok=True)
...
if __name__ == "__main__":
    ...
    if args.train:
        os.makedirs("lilim_PDPTW_train", exist_ok=True)
        os.makedirs("lilim_PDPTW_val", exist_ok=True)
        for n_nodes in range(200,201,100):
            cpl = [[893,476],[160,159],[337,90]] 
            n_samples = 2 * 120000 // n_nodes
            for mu,sig in cpl:
                generate_dataset(n_samples, n_nodes, "lilim_PDPTW_train", mu, sig) 
        for n_nodes in [100, 200]:
            n_samples = 100
            generate_dataset(n_samples, n_nodes, "lilim_PDPTW_val", 68, 101)
    if args.test:
