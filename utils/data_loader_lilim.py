...
class DataLoader(object):
    ...
    def load_data(self, index):
        self.dataset = []
        loading_datasets = []
        couples = [[68,101],[341,483],[618,554],[893,476],[160,159],[337,90]] 
        for i in range(self.n_ranges):
            nodes = [100,200]
            for j in range(len(nodes)):
                if self.problem == "pdp" :
                    n_nodes = 46 + 6 * i + index
                elif self.problem == "pdptw":
                    n_nodes = nodes[j]
                    n_couples = couples[i]
    ...
    def next_batch(self):
        if self.problem == "tsp":
            ...
            assert self.batch_index < 40 * 26
        elif self.problem == "pdptw" :
            assert self.batch_index < 160 * 6
        ...
        if self.problem == "tsp":
            ...
            batch_size = dataset["node_feat"].shape[0] // 40
        elif self.problem == "pdptw":
            batch_size = dataset["node_feat"].shape[0] // 160
