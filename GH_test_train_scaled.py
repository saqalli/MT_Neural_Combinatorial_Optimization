...
def write_para(instance_name, method, para_filename, opt_value, max_trials=1000):
    with open(para_filename, "w") as f:
        ...
        if method == "NeuroLKH":
            f.write("SEED = 1234\n")
            f.write("CANDIDATE_FILE = result/gh_cvrptw/candidate/" + instance_name + ".txt\n")
...
def solve_LKH(instance_name, opt_value, rerun=False, max_trials=1000):
    para_filename = "result/gh_cvrptw/LKH_para/" + instance_name + ".para"
    log_filename = "result/gh_cvrptw/LKH_log/" + instance_name + ".log"
...
def infer_SGN_write_candidate(net, instance_names):
    for instance_name in instance_names:
        ...
        with open("result/gh_cvrptw/candidate/" + instance_name + ".txt", "w") as f:
...
def solve_NeuroLKH(instance_name, opt_value, rerun=False, max_trials=1000):
    para_filename = "result/gh_cvrptw/NeuroLKH_para/" + instance_name + ".para"
    log_filename = "result/gh_cvrptw/NeuroLKH_log/" + instance_name + ".log"
    if rerun or not os.path.isfile(log_filename):
...
def eval_dataset(instance_names, method, args, opt_values, rerun=True, max_trials=1000):
    os.makedirs("result/gh_cvrptw/" + method + "_para", exist_ok=True)
    os.makedirs("result/gh_cvrptw/" + method + "_log", exist_ok=True)
    if method == "NeuroLKH":
        os.makedirs("result/gh_cvrptw/candidate", exist_ok=True)
